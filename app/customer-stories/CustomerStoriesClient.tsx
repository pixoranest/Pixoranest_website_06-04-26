    ...
    <main className="cs-page" itemScope itemType="https://schema.org/WebPage">
      ...
      <div style={{ display: "none" }} aria-hidden="true">
        ...
        <span itemProp="description">
          ...
          CRM workflow automation, and AI voice agents. Read real case studies from Healthcare, Real Estate,
          E-commerce, Finance, Logistics and Hospitality sectors across India.
        </span>
      </div>
      ...
      <section
        className="feat-sec container"
        id="stories"
        aria-label="Featured customer success story"
        itemScope itemType="https://schema.org/Article"
      >
        <p className="eyebrow" aria-hidden="true">Featured Story</p>

        <h2 style={{ position: "absolute", width: "1px", height: "1px", overflow: "hidden", clip: "rect(0,0,0,0)" }}>
          NoBroker.com — Featured WhatsApp Automation Case Study
        </h2>

        <article className="feat-card" itemScope itemType="https://schema.org/Article">
          ...
          <div className="feat-body">
            ...
            <div className="feat-metrics" role="list" aria-label="Key results from this automation case study">
            ...
            <div className="feat-foot">
              ...
              <Link href="#" className="feat-btn" prefetch={false} aria-label={`Read full case study: ${featuredStory.title}`}>
                Read Full Story →
              </Link>
            </div>
          </div>
      ...
      >
        ...
        <p className="eyebrow" aria-hidden="true">All Stories</p>
        ...
        <nav className="filter-bar" role="group" aria-label="Filter case studies by industry">
          {categories.map(cat => (
            <button
              ...
              aria-label={cat === "All" ? "Show all case studies" : `Filter by ${cat} automation case studies`}
            >
              {cat}
              {cat !== "All" && (
                <span className="fcount" aria-label={`${stories.filter(s => s.category === cat).length} stories`}>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="sgrid" role="list" aria-label="Customer case study cards">
            {filtered.map((story, idx) => (
              <div key={story.id} role="listitem" itemProp="itemListElement" itemScope itemType="https://schema.org/ListItem">
      ...
      <section style={{ display: "none" }} aria-hidden="true">
        ...
        <p>
          PixoraNest is India's leading WhatsApp automation and AI calling platform trusted by 500+ businesses.
          Our customers include top healthcare companies using AI receptionists, real estate brands automating
          property lead generation on WhatsApp, e-commerce businesses recovering abandoned carts with automated
          messages, banks deploying conversational AI for customer queries, logistics companies sending automated
          delivery updates, and hospitality businesses reducing support tickets with AI chatbots.
        </p>
      </section>
      ...
      >
        ...
        <div className="container testi-inner">
          <p className="eyebrow testi-ey" aria-hidden="true">Client Voices</p>
          ...
          <div className="tgrid">
            {testimonials.map((t, i) => (
              ...
              >
                ...
                <div className="tcard-q" aria-hidden="true">"</div>
      ...
      <section className="cta-sec" aria-label="Get started with PixoraNest AI automation — book a free demo">
        ...
        <div className="container cta-inner">
          ...
          <div className="cta-proof" aria-label="Key benefits of PixoraNest automation platform">
          ...
          <div className="cta-btns">
            <Link href="/contact" className="cta-primary" prefetch={false} aria-label="Book a free demo with PixoraNest AI automation">
              Book a Free Demo →
            </Link>
            <Link href="/solutions" className="cta-secondary" prefetch={false} aria-label="Explore PixoraNest automation solutions">
              Explore Solutions
            </Link>
          </div>
        </div>
      ...
      <style jsx>{`
        .cs-page { font-family: 'Plus Jakarta Sans', system-ui, sans-serif; color: #1e293b; overflow-x: hidden; background: #f0f6ff; }
        ...
        .sec-title { font-size: clamp(26px,3vw,36px); font-weight: 900; color: #0f172a; margin: 0 0 10px; letter-spacing: -0.025em; }
        .sec-sub { font-size: 15px; color: #64748b; margin: 0 0 32px; line-height: 1.7; }
        .feat-sec { padding: 48px 20px 0; }
        @media (min-width: 640px) { .feat-sec { padding: 72px 28px 0; } }
        .feat-card { display: grid; grid-template-columns: 1fr; background: rgba(255,255,255,0.7); backdrop-filter: blur(20px); border-radius: 20px; overflow: hidden; border: 1px solid rgba(37,99,235,0.12); box-shadow: 0 28px 80px rgba(37,99,235,0.08); }
        @media (min-width: 768px) { .feat-card { grid-template-columns: 1fr 1fr; border-radius: 24px; } }
        .feat-img-wrap { position: relative; height: 240px; overflow: hidden; background: linear-gradient(135deg,#dbeafe,#e0e7ff); }
        @media (min-width: 768px) { .feat-img-wrap { height: 480px; } }
        .feat-card:hover :global(.feat-img-el) { transform: scale(1.05); }
        .feat-img-ov { position:absolute; inset:0; z-index:3; background:linear-gradient(135deg,rgba(15,23,42,0.3),rgba(15,23,42,0.05) 60%,transparent); pointer-events:none; }
        .feat-star-badge { position:absolute; top:16px; left:16px; z-index:4; display:flex; align-items:center; gap:5px; background:rgba(255,255,255,0.9); backdrop-filter:blur(8px); border:1px solid rgba(217,119,6,0.3); color:#d97706; font-size:11px; font-weight:700; padding:4px 10px; border-radius:100px; }
        .feat-body { padding: 24px; display:flex; flex-direction:column; gap:18px; }
        @media (min-width: 640px) { .feat-body { padding: 36px; gap: 22px; } }
        @media (min-width: 1024px) { .feat-body { padding: 44px; } }
        .feat-co-row { display:flex; align-items:center; gap:10px; flex-wrap: wrap; }
        .feat-logo { width:40px; height:40px; border-radius:10px; display:flex; align-items:center; justify-content:center; color:#fff; font-weight:900; font-size:13px; flex-shrink:0; box-shadow:0 4px 14px rgba(0,0,0,0.15); }
        .feat-co-name { font-weight:800; font-size:14px; color:#0f172a; }
        ...
        .feat-title { font-size: clamp(16px, 2vw, 22px); font-weight:900; line-height:1.35; color:#0f172a; margin:0; }
        .feat-excerpt { font-size:14px; line-height:1.75; color:#475569; margin:0; }
        .feat-metrics { display:flex; gap:8px; flex-wrap: wrap; }
        .fm { flex:1; min-width: 80px; background:rgba(37,99,235,0.05); border-radius:12px; padding:14px 10px; display:flex; flex-direction:column; align-items:center; border:1px solid rgba(37,99,235,0.1); transition:all 0.2s; }
        .fm:hover { background:rgba(37,99,235,0.1); border-color:rgba(37,99,235,0.25); transform:translateY(-2px); }
        .fm-v { font-size:22px; font-weight:900; color:#2563eb; line-height:1; letter-spacing:-0.02em; display:block; }
        ...
        .feat-av { width:38px; height:38px; border-radius:50%; display:flex; align-items:center; justify-content:center; color:#fff; font-weight:800; font-size:13px; flex-shrink: 0; }
        .feat-an { font-weight:700; font-size:13px; color:#0f172a; }
        .feat-ar { font-size:11px; color:#94a3b8; }
        .feat-btn { background:linear-gradient(135deg,#1d4ed8,#3b82f6); color:#fff; padding:10px 20px; border-radius:10px; font-size:13px; font-weight:800; text-decoration:none; box-shadow:0 4px 16px rgba(37,99,235,0.3); transition:all 0.2s; white-space:nowrap; }
        .feat-btn:hover { transform:translateY(-1px); box-shadow:0 10px 28px rgba(37,99,235,0.4); }
        .grid-sec { padding: 48px 20px 64px; }
        @media (min-width: 640px) { .grid-sec { padding: 72px 28px 80px; } }
        .filter-bar { display:flex; flex-wrap:wrap; gap:7px; margin-bottom:32px; }
        .fbtn { display:inline-flex; align-items:center; gap:5px; padding:7px 14px; border-radius:100px; border:1.5px solid rgba(37,99,235,0.2); background:rgba(255,255,255,0.75); color:#475569; font-size:12px; font-weight:700; cursor:pointer; transition:all 0.2s; font-family:inherit; backdrop-filter:blur(8px); }
        @media (min-width: 640px) { .fbtn { padding: 8px 18px; font-size: 13px; } }
        ...
        .sgrid { display:grid; grid-template-columns:1fr; gap:20px; }
        ...
        .testi-sec { position:relative; padding:64px 0; overflow:hidden; }
        @media (min-width: 640px) { .testi-sec { padding: 88px 0; } }
        .testi-bg { position:absolute; inset:0; background:linear-gradient(135deg,rgba(219,234,254,0.7) 0%,rgba(238,242,255,0.9) 50%,rgba(219,234,254,0.7) 100%); }
        ...
        .tgrid { display:grid; grid-template-columns:1fr; gap:16px; margin-top:32px; }
        @media (min-width: 768px) { .tgrid { grid-template-columns:repeat(2,1fr); gap:20px; } }
        @media (min-width: 1024px) { .tgrid { grid-template-columns:repeat(3,1fr); gap:24px; } }
        .tcard { background:rgba(255,255,255,0.78); border:1px solid rgba(37,99,235,0.1); border-radius:18px; padding:24px; backdrop-filter:blur(12px); display:flex; flex-direction:column; gap:14px; transition:all 0.3s; margin:0; }
        @media (min-width: 640px) { .tcard { padding: 32px; } }
        ...
        .tcard-q { font-size:52px; font-family:Georgia,serif; line-height:0.6; color:#2563eb; display:block; }
        .tcard-text { font-size:14px; line-height:1.75; color:#334155; margin:0; font-style:italic; flex:1; }
        .tcard-auth { display:flex; align-items:center; gap:10px; }
        .tcard-av { width:42px; height:42px; border-radius:50%; display:flex; align-items:center; justify-content:center; color:#fff; font-weight:800; font-size:13px; flex-shrink:0; }
        ...
        .cta-sec { position:relative; padding:64px 0 80px; text-align:center; overflow:hidden; background:#f0f6ff; }
        @media (min-width: 640px) { .cta-sec { padding: 88px 0 100px; } }
        ...
        .cta-inner { position:relative; z-index:1; padding: 0 20px; }
        .cta-icon { width:60px; height:60px; border-radius:50%; background:rgba(37,99,235,0.1); border:1px solid rgba(37,99,235,0.2); display:flex; align-items:center; justify-content:center; margin:0 auto 20px; }
        .cta-title { font-size:clamp(26px,4vw,46px); font-weight:900; color:#0f172a; margin:0 0 14px; letter-spacing:-0.03em; line-height:1.1; }
        ...
        .cta-sub { font-size:15px; color:#64748b; margin:0 auto 24px; max-width:480px; line-height: 1.7; }
        .cta-proof { display:flex; flex-wrap:wrap; gap:10px; justify-content:center; margin-bottom:28px; }
        ...
        .cta-primary { background:linear-gradient(135deg,#1d4ed8,#3b82f6); color:#fff; padding:13px 26px; border-radius:12px; font-weight:800; font-size:14px; text-decoration:none; box-shadow:0 8px 24px rgba(37,99,235,0.3); transition:all 0.2s; }
        @media (min-width: 640px) { .cta-primary { padding: 15px 30px; font-size: 15px; } }
        .cta-primary:hover { transform:translateY(-2px); box-shadow:0 16px 40px rgba(37,99,235,0.4); }
        .cta-secondary { background:rgba(255,255,255,0.85); color:#334155; padding:13px 24px; border-radius:12px; font-weight:800; font-size:14px; text-decoration:none; border:1.5px solid rgba(37,99,235,0.2); backdrop-filter:blur(8px); transition:all 0.2s; }
        @media (min-width: 640px) { .cta-secondary { padding: 15px 30px; font-size: 15px; } }
        .cta-secondary:hover { background:rgba(255,255,255,0.95); transform:translateY(-2px); border-color:rgba(37,99,235,0.35); }
        @media (max-width: 400px) { .cta-btns { flex-direction: column; align-items: center; } .cta-primary, .cta-secondary { width: 100%; max-width: 300px; text-align: center; } }
